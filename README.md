...
python fig1.py     # Figure 1   A, C          ideal vs distorted geometry
python figS1.py    # Suppl. S1  B, C, D       PARSE/DSSR disagreement types
python fig5.py     # Figure 5   C, D          before/after PDB-REDO
python fig6.py     # Figure 6   A, B, C       quality mapped onto a structure
```

Each opens its own PyMOL with the panels prepared; call `panel("1A")`,
`panel("S1B")` and so on, then save the session (**File > Save Session As…**).
The coordinates every panel needs are committed under
`data/figures/structures/` as minimal extracts, so nothing is downloaded at run
time. Figure 6 additionally needs the compiled engine (see Level 3), because it
is generated by the shipped PyMOL integration rather than by a script of its own.

See `scripts/pymol_figures/README.md` for the panel list, what each one shows,
and how to regenerate the vendored coordinates.
